...
function sideFor(p, d) {
  ...
  if (p.treatment === 'scan' || p.treatment === 'auto') {
    ...
    if (p.state === 'blocked') {
      ...
      block = `<p class="pdl-note">${esc(p.note)}${p.needs_article ? ({ x: '。发的是研习室那篇文章，横版封面放最前面；需要 X Premium', wechat_mp: '。发的是研习室那篇文章，自动排版（橄榄手记）+ 公众号封面；「发布」不推送粉丝', miniprogram: '。发的是「研习室文章」那一篇，排版和网页后台导入一样；再发一次会更新同一篇' }[p.key] || '') : p.no_video ? '。发的是文字，不带视频' : `。会上传 ${esc(d.video.name)}（${d.video.mb} MB）${p.key === 'bilibili' && d.release && d.release.covers && d.release.covers.landscape ? '，封面用横版封面' : ''}`}。</p>
