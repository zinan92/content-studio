...
3. why_boom 与 why_scatter：各 2–4 条，每条 text 必须至少引用一个上面“数据”里的数字，并落到转写里的具体做法；evidence_start 填支撑它的原文秒数。
...
def validate_judgement(raw: dict[str, Any], duration: float, needs_opening: bool) -> list[str]:
    ...
    for key in ("why_boom", "why_scatter"):
        ...
        for index, item in enumerate(items):
            ...
            elif not _has_number(text):
                problems.append(f"{key}[{index}] 没有引用任何数字")
    if needs_opening:
